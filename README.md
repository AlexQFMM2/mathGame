...
项目不会被命名和架构限定为“数独 App”。数独是第一款游戏，也是用来验证棋盘交互、可解释提示、存档和 Android 封装的起点；算术填字是第二款游戏，用来验证多游戏目录、共享活动系统和非规则棋盘玩法。

## 当前状态

Web 端已经可以完整游玩数独和算术填字：
...
- [x] 两款游戏结算页展示完成棋盘、复制题目编号，并可按编号再次挑战
- [x] 算术填字四档难度、14×14 支流地图、数字/符号卡牌和变量推导
- [x] 算术填字保证至少一解并允许多解，填满错误时只退回冲突支流卡牌
- [ ] Android 真机触觉反馈和生命周期验收

当前 Web 主流程为：主页 → 选择游戏 → 选择难度或按种子进入 → 本机生成题目 → 游戏 → 结算。数独生成器验证唯一解；算术填字生成器先创建有序线性关系模板，再布局并联立求出确定性的 `14×14` 关系网，验证解数量 `>= 1`，不排除数学上合理的多解。两款游戏都能通过题号和 seed 重新生成完全相同的题目。
...
└── plan/
    ├── README.md            计划索引和维护规则
    └── current/
        └── arithmetic-crossword-mvp-plan.md
```
...
- [计划索引](plan/README.md)
- [算术填字 MVP 计划](plan/current/arithmetic-crossword-mvp-plan.md)
- [已归档计划](plan/archive/)
